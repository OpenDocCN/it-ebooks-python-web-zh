...
不需要在我们自己的服务器处理大量的上传图片，我们依赖 [Gravatar](http://gravatar.com/) [http://gravatar.com/] 服务为我们提供用户头像。
...
有了 Gravatar 服务的协助，很容易处理头像。你只需要创建一个用户邮箱的 MD5 哈希，然后将其加入 URL 中，像上面你看见的。在邮箱 MD5 后，你还需要提供一个定制头像尺寸的数字。*d=mm* 决定什么样的图片占位符当用户没有 Gravatar 账户。*mm* 选项将会返回一个“神秘人”图片，一个人灰色的轮廓。*s=N* 选项要求头像按照以像素为单位的给定尺寸缩放。

[Gravatar 官方文档](https://gravatar.com/site/implement/images) [https://gravatar.com/site/implement/images] 对 avatar URL 有着更加详细的解释。
...
![_images/5.png](img/5.png)

## 结束语

最后留给大家一个问题，应用程序存在一个 bug。这个问题在前面的章节就已经存在，这一章的代码存在同样的问题。在下一章中我会解释这个 bug，并且修正它。

如果你想要节省时间的话，你可以下载 [microblog-0.6.zip](https://github.com/miguelgrinberg/microblog/archive/v0.6.zip) [https://github.com/miguelgrinberg/microblog/archive/v0.6.zip]。
