...
紧跟着错误后面的是错误的 [堆栈跟踪](http://en.wikipedia.org/wiki/Stack_trace) [http://en.wikipedia.org/wiki/Stack_trace]，这是一个相当不错的东西，在这里你可以去任何一帧并且检查源代码或者甚至在浏览器正确地上计算表达式。
...
目前为止在测试框架中有两个测试。第一个就是验证 Gravatar 的头像 URL 生成是否正确。注意测试中期待的结果是硬编码，验证 *User* 类的返回的头像 URL。
...
如果你想要节省时间的话，你可以下载 [microblog-0.7.zip](https://github.com/miguelgrinberg/microblog/archive/v0.7.zip) [https://github.com/miguelgrinberg/microblog/archive/v0.7.zip]。
