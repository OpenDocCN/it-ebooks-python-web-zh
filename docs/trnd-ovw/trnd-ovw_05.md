...
这样配置后，所有以 `/static/` 开头的请求，都会直接访问到指定的静态文件目录， 比如 [`localhost:8888/static/foo.png`](http://localhost:8888/static/foo.png) 会从指定的静态文件目录中访问到 `foo.png` 这个文件。同时 `/robots.txt` 和 `/favicon.ico` 也是会自动作为静态文件处理（即使它们不是以 `/static/` 开头）。
